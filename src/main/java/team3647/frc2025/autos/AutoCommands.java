...
public class AutoCommands implements AllianceObserver {
	...
	public Command scorePreload() {
		return Commands.sequence(
				Commands.parallel(
						superstructure.wristCommands.setAngle(Degree.of(95))),
				Commands.sequence(
						...
						Commands.parallel(
								...
								superstructure.pivotCommands.setAngle(PivotConstants.kStowAngleUp))),
	...
	public Command getSuperstructureTwoS3_e1f2() {
		return Commands.sequence(
				...
				superstructure.wristCommands.setAngle(WristConstants.kSourceIntakeAngle)
						.alongWith(superstructure.rollersCommands.setOpenLoop(-0.3))
						.until(superstructure.intakeCurrent()),
				superstructure.stowFromIntake().alongWith(autoDrive.setWantedScoringPos(ScoringPos.F2)),
	...
	public AutoCommands(SwerveDrive swerve, Superstructure superstructure, AutoDrive autoDrive) {
		...
		this.blueOne_s2d2 = new AutoMode(getOneS2_d2(), getInitial(s2_to_d2, Alliance.Blue), "blue one Piece mid");
		this.DT_One_s2d2 = new AutoMode(getOneDT_s2e2(), getInitial(s2_to_d2, Alliance.Blue),
				"blue one piece mid ONLY DT");

		this.redOne_s2d2 = new AutoMode(getOneS2_d2(), getInitial(s2_to_d2, Alliance.Red), "red one Piece mid");

		blueAutosList = List.of(new AutoMode(Commands.none(), new Pose2d(), "nothingBlue"), blueOne_s2d2, DT_One_s2d2);
		redAutosList = List.of(new AutoMode(Commands.none(), new Pose2d(), "nothingRed"), redOne_s2d2, DT_One_s2d2);
