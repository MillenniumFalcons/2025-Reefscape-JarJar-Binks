...
public class AutoCommands implements AllianceObserver {
	...
	public Command followChoreoPath(Trajectory<SwerveSample> traj){
		return customPathFollower(traj, 
		choreoSwerveController(autoConstants.xController, autoConstants.yController, autoConstants.rotController), 
		(speeds) -> {
		...
		return new FunctionalCommand(
			timer::restart, () -> {
				outputChassisSpeeds.accept(
					controller.apply(
						poseSupplier.get(), 
						traj.sampleAt(timer.get(), isRed()).orElse(autoConstants.kEmptySample))
				);
			}, 
			(interrupted) -> {outputChassisSpeeds.accept(new ChassisSpeeds()); timer.stop();},
			() -> PoseUtils.inCircle(poseSupplier.get(), traj.getFinalPose(isRed()).get(), 0.1), swerve);
	}
	...
	public List<AutoMode> redAutosList = List.of(new AutoMode(Commands.none(), new Pose2d(), "nothingRed"));
	public List<AutoMode> blueAutosList = List.of(new AutoMode(Commands.none(), new Pose2d(), "nothingBlue"));
