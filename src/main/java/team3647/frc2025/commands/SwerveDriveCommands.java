...
public class SwerveDriveCommands implements AllianceObserver {
	...
	public Command driveCmd(
		...
		return Commands.run(() -> {
			...
			double ySquared = 
					Math.pow(y.getAsDouble(), 2)
					* Math.signum(y.getAsDouble())
					* 1.05;
			double xSquared = 
					Math.pow(x.getAsDouble(), 2)
					* Math.signum(x.getAsDouble())
					* 1.05;
			...
			double motionTurnComponent = rot.getAsDouble() * -1 * kMaxSpeed.in(MetersPerSecond);
	...
	// Supplier<Twist2d> autoSteerVelocitiesSupplier
		...
		return Commands.run(
				() -> {
					...
					double y = Math.pow(ySpeedFunction.getAsDouble(), 2)
							* Math.signum(ySpeedFunction.getAsDouble())
							* 1.05;
					double x = Math.pow(xSpeedFunction.getAsDouble(), 2)
