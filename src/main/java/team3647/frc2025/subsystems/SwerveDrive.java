...
public class SwerveDrive extends SwerveDrivetrain<TalonFX, TalonFX, CANcoder>
	...
	public SwerveDrive(
		...
		this.spinSysIdRoutine = new SysIdRoutine(
				new SysIdRoutine.Config(
				...
				new Mechanism(
						(Voltage volts) -> setControl(
								periodicIO.spinSysidRequest.withRotationalRate(volts.in(Units.Volts))),
						null,
						this));
	...
	public void reset() {
		for (int i = 0; i < 4; ++i) {
			periodicIO.setpoint.mModuleStates[i] = new team3647.lib.team254.swerve.SwerveModuleState(
					0.0,
