...
public class Superstructure {
	...
	public SuperstructureState getWantedStateByLevel(Level level){
		switch (level) {
			case TROUGH:
				return new SuperstructureState(PivotConstants.kLevel1Angle, ElevatorConstants.kLevel1Height, -0.7);
			case LOW:
				return new SuperstructureState(PivotConstants.kLevel2Angle, ElevatorConstants.kLevel2Height, -0.7);
			case MID:
				return new SuperstructureState(PivotConstants.kLevel3Angle, ElevatorConstants.kLevel3Height, -0.7);
			case HIGH:
				return new SuperstructureState(PivotConstants.kLevel4Angle, ElevatorConstants.kLevel4Height, -0.7);
			case INTAKE:
				return new SuperstructureState(PivotConstants.kIntakeAngle, ElevatorConstants.kIntakeHeight, 1);
			default:
				return new SuperstructureState(PivotConstants.kStowAngle, ElevatorConstants.kStowHeight, 0);
		}
	}
	...
	public Command goToStatePerpendicular(SuperstructureState state){
		return Commands.sequence(
			...
			pivotCommands.setAngle(state.pivotAngle)
				.alongWith(coralerCommands.setOpenLoop(state.rollers)).withTimeout(Units.Seconds.of(0.6))
		);
	}
